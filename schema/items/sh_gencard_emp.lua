ITEM.name = "Нейронная карта - \"EMP\" "
ITEM.description = "Оцифрованная запись чьего-то мозга, чей конструкт имел довольно хорошую эмпатию. Можно применить в БИСК-М."
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if IsValid(entity) and entity:GetClass() == "ix_lvlupcapsule" then 
            ...
            local current = character:GetAttribute("emp", 0)
            local debaf = character:GetAttribute("dex", 0)

            if current < 20 then
                character:SetAttrib("dex", debaf - 1)
                character:SetAttrib("emp", current + 1)
