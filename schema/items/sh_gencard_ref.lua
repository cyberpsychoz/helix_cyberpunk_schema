ITEM.name = "Генетическая карта - \"REF\" "
ITEM.description = "Оцифрованная запись чьего-то генетического кода, чья оболочка имела довольно хорошие рефлексы. Можно применить в БИСК-М."
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if IsValid(entity) and entity:GetClass() == "ix_lvlupcapsule" then 
            ...
            local current = character:GetAttribute("ref", 0)
            local debaf = character:GetAttribute("phy", 0)
            if current < 20 then
                character:SetAttrib("phy", debaf - 1)
                character:SetAttrib("ref", current + 1)
