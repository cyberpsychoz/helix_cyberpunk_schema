ITEM.name = "Генетическая карта - \"DEX\" "
ITEM.description = "Оцифрованная запись чьего-то генетического кода, чья оболочка имела довольно хорошую ловкость. Можно применить в БИСК-М."
...
ITEM.functions.Use = {
    ...
    OnRun = function(item)
        ...
        if IsValid(entity) and entity:GetClass() == "ix_lvlupcapsule" then 
            ...
            local current = character:GetAttribute("dex", 0)
            local debaf = character:GetAttribute("ref", 0)

            if current < 20 then
                character:SetAttrib("ref", debaf - 1)
                character:SetAttrib("dex", current + 1)
