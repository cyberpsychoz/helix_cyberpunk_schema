						...
						if v[1] == "Combat" then
							if IsEntity(v[v[5]]) and IsValid( v[v[5]] ) then
								if v[v[5]]:GetClass() == "player" then
									...
									if v[v[5]]:KeyPressed( IN_WALK ) and IsValid(v[v[5]]) and v[v[5]]:Alive() then -- and IsValid(v[v[5]]:GetActiveWeapon()) then -- нахуя оно мне тут
										-- if the player uses AP then skips it's not considered tryingtoleave
										if v[v[5]]:GetNWInt("AP", 3) == ix.config.Get("playeractionpoints", 3) + char:GetAttribute("ref", 0) then
