resource.AddFile("resource/fonts/New_Zelek.ttf")
...
function PANEL:Init()
    ...
    local background1 = vgui.Create("DImage", frame)
    background1:SetSize(ScrW(), ScrH())
    background1:SetImage(images[index])
    background1:SetAlpha(255)

    local background2 = vgui.Create("DImage", frame)
    background2:SetSize(ScrW(), ScrH())
    background2:SetAlpha(0)

    -- Таймер для смены изображений
    timer.Create("ChangeBackground", 15, 0, function()
        if not IsValid(background1) or not IsValid(background2) then return end

        index = math.random(#images)
        background2:SetImage(images[index])

        -- Анимация плавного перехода
        local anim = Derma_Anim("Fade", background2, function(panel, anim, delta, data)
            panel:SetAlpha(delta * 255)
        end)
        anim:Start(2) -- Плавный переход в течение 2 секунд

        -- Обновление анимации каждый кадр
        background2.Think = function(panel)
            if anim:Active() then
                anim:Run()
            else
                background1:SetImage(images[index])
	...
	self.mainButtonList:SetPos(100, titleImage:GetTall() + 250)
	...
	createButton:SetFont("CyberpunkFont")
	createButton:SizeToContents()
	createButton:SetTextColor(Color(194, 194, 194))
	createButton.DoClick = function()
		...
		if IsValid(background1) then
			background1:Remove()
		end
		if IsValid(background2) then
			background2:Remove()
		end
	...
	local buttonPanel = self.mainButtonList:Add("DPanel")
	buttonPanel:SetSize(300, 50) -- увеличиваем ширину панели

	-- добавляем изображение на панель
	local buttonImage = buttonPanel:Add("DImage")
	buttonImage:SetSize(300, 50) -- увеличиваем ширину изображения
	buttonImage:SetImage("materials/button1.png") -- устанавливаем изображение

	-- добавляем текст на панель поверх изображения
	local buttonText = buttonPanel:Add("DLabel")
	buttonText:SetSize(350, 50) -- увеличиваем ширину текста
	buttonText:SetFont("CyberpunkFont") -- устанавливаем шрифт
	buttonText:SetText("СОЗДАТЬ ПЕРСОНАЖА") -- устанавливаем текст
	buttonText:SetContentAlignment(5) -- центрируем текст
	buttonText:DockMargin(10, 0, 10, 0) -- добавляем отступы слева и справа

	-- добавляем функцию клика на панель
	...
	self.loadButton:SetFont("CyberpunkFont")
	self.loadButton:SizeToContents()
	self.loadButton:SetTextColor(Color(194, 194, 194))
	...
	if (extraURL != "" and extraText != "") then
		...
		extraButton:SetText("ДИСКОРД")
		extraButton:SetFont("CyberpunkFont")
		extraButton:SetTextColor(Color(194, 194, 194))
	...
	self.returnButton:SetFont("CyberpunkFont")
	self.returnButton:SetTextColor(Color(194, 194, 194))
...
function PANEL:UpdateReturnButton(bValue)
	...
	self.returnButton:SetText(self.bUsingCharacter and "ВЕРНУТЬСЯ" or "УБЕЖАТЬ")
