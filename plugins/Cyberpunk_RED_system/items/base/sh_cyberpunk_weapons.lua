...
properties.Add("target_capture", {
    ...
    Order = 1000,
    MenuIcon = "materials/icons/1_2.png",

    Filter = function(self, ent, ply)
        -- Здесь вы можете добавить условия для отображения кнопки
        -- Например, показывать кнопку только для определенных объектов
        return IsValid(ent)
    end,

    Action = function(self, ent)
        -- Здесь вы можете добавить действия, которые будут выполнены при нажатии кнопки
        -- Например, захватить цель
        self:MsgStart()
            net.WriteEntity(ent)
        self:MsgEnd()
    end,
    ...
    Receive = function(self, length, ply)
        ...
        -- Здесь вы можете добавить действия, которые будут выполнены на сервере при получении сообщения
        -- Например, захватить цель
    end
})
