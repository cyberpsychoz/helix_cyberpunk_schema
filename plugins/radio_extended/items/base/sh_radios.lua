...
function ITEM:GetDescription()
	...
	local displayFreq = string.format(" и %s %s MHz", duplexTrue and "получение на" or "настроенный на", duplexTrue and self:GetData("listenfrequency", "100.0") or self:GetData("frequency","100.0"))
	local ret = string.format(self.description, notWalkie and " с частотным тюнером" or "", enabled and "ВКЛ" or "ВЫКЛ", (enabled and notWalkie) and displayFreq or "")
...
function ITEM.postHooks.Scan(item, status)
	...
	if !lockedOn then
		...
		character:SetData("frequency",randFreq)
		character:SetData("channel","1")
		itemTable.player:Notify("Зафиксирован слабый сигнал на канале 1.")
	end
...
ITEM.functions.Broadcast = {
	name = "Режим вещания по всем каналам",
	OnRun = function(itemTable)
		...
		if itemTable:GetData("broadcast") then
			itemTable.player:NotifyLocalized("Теперь вы транслируете по всем каналам на частоте "..itemTable:GetData("frequency","100.0").." MHz.")
		else
			itemTable.player:NotifyLocalized("Вы больше не транслируете по всем каналам.")
		end
...
ITEM.functions.AdMode = { -- Sorry, ordering
	...
	OnRun = function(itemTable)
		...
		itemTable.player:Notify(string.format("Ваше радио теперь работает на частоте %s.%s", itemTable:GetData("duplex",itemTable.duplex) and "duplex mode" or "simplex mode",(itemTable:GetData("storefrequency") and itemTable:GetData("duplex")) and "\nВаша частота восстановлена." or ""))
		return false
	end,
	...
	OnRun = function(itemTable)
		...
		if (itemTable:GetData("enabled")) then
			...
			if itemTable:GetData("silenced") then
				itemTable.player:NotifyLocalized("Вы заглушили радио.")
			else
				itemTable.player:NotifyLocalized("Вы вернули радио в прежний режим работы.")
			end
		end
		return false
	end,
...
ITEM.functions.Listen = {
	name = "Режим прослушки по всем каналам",
	OnRun = function(itemTable)
